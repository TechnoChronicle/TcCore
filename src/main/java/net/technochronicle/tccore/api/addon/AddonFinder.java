...
public class AddonFinder {
    ...
    private static final List<ITcAddon> cache = new ArrayList<>();
    private static Map<String, ITcAddon> modIdMap = null;

    @UnmodifiableView
    public static List<ITcAddon> getAddonList() {
        ensureInitialized();
        return Collections.unmodifiableList(cache);
    }

    @UnmodifiableView
    public static Map<String, ITcAddon> getAddons() {
        ensureInitialized();
        return Collections.unmodifiableMap(modIdMap);
    }

    @Nullable
    public static ITcAddon getAddon(String modId) {
    ...
    private static Map<String, ITcAddon> getInstances() {
        ...
        for (IModInfo modInfo : allMods) {
            ...
            scanData.getAnnotatedBy(TcAddon.class, ElementType.TYPE)
        ...
        Map<String, ITcAddon> instances = new LinkedHashMap<>();
        for (var entry : addonClassNames.entrySet()) {
            String modId = entry.getKey();
            String className = entry.getValue();
            try {
                Class<?> asmClass = Class.forName(className);
                Class<? extends ITcAddon> asmInstanceClass = asmClass.asSubclass(ITcAddon.class);
                try {
                    Constructor<? extends ITcAddon> constructor = asmInstanceClass.getDeclaredConstructor();
                    ITcAddon instance = constructor.newInstance();
                ...
                LOGGER.error("Failed to load: {} from {}, does not extend ITcAddon!", className, modId, e);
