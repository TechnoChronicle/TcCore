...
public class FluidStorageImpl implements FluidStorage {
     ...
     */
    @ApiStatus.Internal
    public void registerFluids(@NotNull Material material, TcRegistrate registrate) {
        if (registered) {
        ...
        toRegister.entrySet().stream()
                ...
                .forEach(entry -> {
                    if (map.containsKey(entry.getKey())) {
                        TcCore.LOGGER.warn("{} already has an associated fluid for material {}", entry.getKey(),
                                material);
                        return;
                    }
                    ...
                    if (!storeNoOverwrites(entry.getKey(), fluid, entry.getValue())) {
                        TcCore.LOGGER.error("{} already has an associated fluid for material {}", material, material);
                    }
                });
